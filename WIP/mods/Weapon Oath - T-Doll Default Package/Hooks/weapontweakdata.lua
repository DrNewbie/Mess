			...
			if type(self[__id].__oath_data) ~= "table" then
				self[__id].__oath_data = {
					...
					__oath_link = function(is_click, now_rate)
						if is_click then
							Steam:overlay_activate("url", "https://iopwiki.com/wiki/"..__pd2_to_tdoll[__id])
							if blt.xaudio then
								local __default_ogg_folder = ThisModPath.."Sounds/Default/"..string.upper(__pd2_to_tdoll[__id]).."/"
								if file.DirectoryExists(__default_ogg_folder) then
									...
									if not table.empty(__oggs) then
										...
										if io.file_is_readable(__this_ogg) then
											...
											--XAudio.Source:new(XAudio.Buffer:new(__this_ogg))
										end
									end
								end
							end
