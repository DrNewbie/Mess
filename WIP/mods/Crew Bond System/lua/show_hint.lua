...
if _G[__Name(102)] then return end
_G[__Name(102)] = true
...
HudCrewBondUSystemNotification = HudCrewBondUSystemNotification or class(ExtendedPanel)
HudCrewBondUSystemNotification.ICON_SIZE = 80
HudCrewBondUSystemNotification.BOX_MAX_W = 400
HudCrewBondUSystemNotification.default_queue = HudCrewBondUSystemNotification.default_queue or {}

function HudCrewBondUSystemNotification.queue(title, text, icon, rewards, queue)
	if Application:editor() then
		return
	end

	queue = queue or HudCrewBondUSystemNotification.default_queue
	...
	if #queue == 1 then
		HudCrewBondUSystemNotification:new(unpack(queue[1]))
	end
end

function HudCrewBondUSystemNotification:init(title, text, icon, rewards, queue)
	if _G.IS_VR then
		HudCrewBondUSystemNotification.super.init(self, managers.hud:prompt_panel())
	else
		self._ws = managers.gui_data:create_fullscreen_workspace()

		HudCrewBondUSystemNotification.super.init(self, self._ws:panel())
	end
	...
	self:set_bottom(self:parent():h() * 3 / 7)
...
function HudCrewBondUSystemNotification:close()
	self:remove_self()
	...
	if #self._queue > 0 then
		HudCrewBondUSystemNotification:new(unpack(self._queue[1]))
	end
end
