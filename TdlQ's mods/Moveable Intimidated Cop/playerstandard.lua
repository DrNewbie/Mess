...
local mic_original_playerstandard_getunitintimidation_action = PlayerStandard._get_unit_intimidation_action
function PlayerStandard:_get_unit_intimidation_action(...)
	_secondary = select(select('#', ...), ...)
	return mic_original_playerstandard_getunitintimidation_action(self, ...)
end

local mic_original_playerstandard_getinteractiontarget = PlayerStandard._get_interaction_target
function PlayerStandard:_get_interaction_target(char_table, my_head_pos, cam_fwd)
	if _secondary then
		for _, u_data in pairs(managers.enemy:all_enemies()) do
			...
			if alive(unit) and unit:base().mic_is_being_moved == self._unit then
				...
				self:_add_unit_to_char_table(char_table, unit, 0, intimidate_range_civ, true, true, 0.01, my_head_pos, cam_fwd)
			end
		end
		_secondary = nil
	end

	return mic_original_playerstandard_getinteractiontarget(self, char_table, my_head_pos, cam_fwd)
end
...
function PlayerStandard:_get_intimidation_action(prime_target, char_table, amount, primary_only, detect_only, secondary)
	...
	if prime_target_base and prime_target_base.mic_is_being_moved == self._unit then
		local t = TimerManager:game():time()
		if not self._intimidate_t or t - self._intimidate_t > tweak_data.player.movement_state.interaction_delay then
			self._intimidate_t = t
			if Network:is_server() then
				target_unit:brain():mic_on_intimidated(self._unit, secondary)
			else
			...
			if secondary then
				...
				local old_dst = prime_target_base.mic_destination
				local same_dst = old_dst and mvector3.equal(wp_position, old_dst)
				if not same_dst then
					prime_target_base.mic_destination = mvector3.copy(wp_position)
				...
				-- call
				if prime_target_base.mic_destination then
					prime_target_base.mic_destination = nil
				end
				_restart(target_unit)
