...
function NetworkPeer:Is_MWS_Member()
	if not self or not self._user_id or not self._name then
		return
	end
	if managers.network and managers.network:session() and self == managers.network:session():local_peer() then
		return
	end
