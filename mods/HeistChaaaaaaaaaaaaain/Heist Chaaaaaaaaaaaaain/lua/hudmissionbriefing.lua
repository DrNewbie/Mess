...
function HUDMissionBriefing:init(hud, workspace)
	...
	for i = 1, (HeistChaaaaaaaaaaaaain.Level_Num) do
		...
		if self._job_schedule_panel:child("day_" .. tostring(i - 1)) then
	...
	for i = 1, managers.job:current_stage() or 0 do
		if self._job_schedule_panel:child("day_" .. tostring(i)) then
	...
	if managers.job:has_active_job() then
		...
		if self._job_schedule_panel:child("day_" .. tostring(num_stages)) then		
