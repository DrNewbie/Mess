Hooks:PostHook(SniperGrazeDamage, "on_weapon_fired", "F_"..Idstring("PostHook:SniperGrazeDamage:on_weapon_fired:Graze Plus - Make graze effect apply of opposite side"):key(), function(self, weapon_unit, result) 
	if not alive(weapon_unit) or not weapon_unit:base():is_category("snp") or weapon_unit ~= managers.player:equipped_weapon_unit() or not result.hit_enemy or not result.rays then
