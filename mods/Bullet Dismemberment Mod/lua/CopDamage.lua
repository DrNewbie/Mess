...
Hooks:PostHook(CopDamage, "damage_bullet", "BD_CopDamagePostDamageBullet", function(self, attack_data)
	...
	if not self._dead or (attack_data.attacker_unit:inventory() and BulletDecapitations.tweak_data.allowed_weapons[tweak_data.weapon[attack_data.attacker_unit:inventory():equipped_unit():base():get_name_id()].category] ~= true) then
