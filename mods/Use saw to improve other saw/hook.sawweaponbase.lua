Hooks:PostHook(SawHit, "on_collision", "UseSaw2ImproveOtherSaw", function(self, col_ray, weapon_unit, user_unit, damage)
	local hit_unit = col_ray and col_ray.unit or nil
	if hit_unit and hit_unit:base() then
		if hit_unit:name() == Idstring("units/pd2_dlc_jolly/equipment/gen_interactable_saw/gen_interactable_saw") then
