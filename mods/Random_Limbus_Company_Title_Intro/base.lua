...
Hooks:Add("MenuManagerOnOpenMenu", Hook1, function(self, menu)
	if menu == "menu_main" then
		DelayedCalls:Add(Hook2, 1, function()
			local __copy = function(this_file, to_this_path)
				if type(this_file) == "string" and io.file_is_readable(this_file) then
					local ThisMoveieFileFrom = io.open(this_file, "rb")
					if ThisMoveieFileFrom then
						local ThisMoveieFileFromData = ThisMoveieFileFrom:read("*all")
						local ThisMoveieTo = ThisModPath..to_this_path
			...
			if type(ThoseMovies) == "table" and not table.empty(ThoseMovies) then
				__copy(ThoseMovies[table.random_key(ThoseMovies)], "/assets/movies/game_intro.movie")
			end
		end)
	end
end)