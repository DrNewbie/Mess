...
if not _G.Sound_Moving then
	...
	function Sound_Moving:Is_ply_OK()
		...
		if not self.Data or not self.Data.ply or not self.Data.ply._unit or not alive(self.Data.ply._unit) then
			return
		end
		return true
	end
...
if PlayerStandard then
	Hooks:PreHook(PlayerStandard, "_update_melee_timers", "F_"..Idstring("PostHook:PlayerStandard:_update_melee_timers:Create_Point"):key(), function(self, t)
		if self._state_data.melee_damage_delay_t and self._state_data.melee_damage_delay_t <= t then
			...
			if col_ray and col_ray.unit and alive(col_ray.unit) and col_ray.hit_position then
				Sound_Moving:Create_Point(col_ray.hit_position)
			end
		end
	end)
end