--https://modworkshop.net/mod/36879
local ThisModPath = ModPath
local ThisModIds = Idstring(ThisModPath):key()
local ThisModSave = SavePath .. "Whitelist-Heists-from-Crime-Net.json"
local __Name = function(__id)
	return "WHCN_"..Idstring(tostring(__id).."::"..ThisModIds):key()
end
...
Hooks:Add("LocalizationManagerPostInit", __Name("LocalizationManagerPostInit"), function(loc)
	LocalizationManager:add_localized_strings({
		[ThisModMenuTitle] = "Whitelist Heists from Crime.Net",
		[ThisModMenuDesc] = " ",
		[ThisModMenuNoDesc] = " "
	})
end)
