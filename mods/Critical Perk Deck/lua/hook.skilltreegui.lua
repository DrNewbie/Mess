...
function SpecializationTierItem:custom_tier_icon(guis_catalog, texture_rect_x, texture_rect_y, texture)
	if texture then
		return {
			layer = 1,
			halign = "scale",
...
function SpecializationTierItem:custom_init(tier_data, tree_panel, tree, tier, x, y, w, h)
	...
	local tier_icon = tier_panel:bitmap(self:custom_tier_icon(guis_catalog, texture_rect_x, texture_rect_y, tier_data.texture or nil))
...
function SpecializationTierItem:init(tier_data, tree_panel, tree, tier, x, y, w, h)
	...
	if tree_tweak.custom and tree_tweak.custom_id == "Critical" and tree_tweak[tier] and tree_tweak[tier].custom then
