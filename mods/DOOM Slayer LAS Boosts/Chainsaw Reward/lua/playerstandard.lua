local Ply_init_XRun_Bonus_Cut_Melee = false

Hooks:PostHook(PlayerStandard, "init", "XXRun_Bonus_Cut_Melee_Init", function(self)
	if Input and Input:keyboard() and not Ply_init_XRun_Bonus_Cut_Melee then
		Ply_init_XRun_Bonus_Cut_Melee = true
		self._Bonus_Cut_Melee_Times_Max = 10
		self._Bonus_Cut_Melee_Times = self._Bonus_Cut_Melee_Times_Max
	end
...
function PlayerStandard:X_is_Bonus_Cut_Melee()
...
function PlayerStandard:XRun_Bonus_Cut_Melee()
	local t = TimerManager:game():time()
	if self._Bonus_Cut_Melee_Times < 0 or self:X_is_Bonus_Cut_Melee() or self:_changing_weapon() or self:_is_reloading() or self:_interacting() or self:_is_meleeing() or self._use_item_expire_t or self:_is_throwing_projectile() or self:_on_zipline() or self:on_ladder() then
		return self:_check_action_melee(t, {btn_melee_release = true})
	end	
	managers.blackmarket:apply_fake_equipped_melee_weapon("cs")
end

Hooks:PostHook(PlayerStandard, "update", "XCheckHoldTheKeyModPress", function(self)
	if HoldTheKey and managers.player:Is_LAS_DewmSlaya() then
		local b_cr_key_1 = HoldTheKey:Get_BLT_Keybind("b_cr_keybind_1")
		if b_cr_key_1 then
			if not self.press__b_cr_key_1 and HoldTheKey:Key_Held(b_cr_key_1) then
				self.press__b_cr_key_1 = true
				self:XRun_Bonus_Cut_Melee()
			end
			if self.press__b_cr_key_1 and not HoldTheKey:Key_Held(b_cr_key_1) then
				self.press__b_cr_key_1 = false
			end
		end
	end
end)

Hooks:PostHook(PlayerStandard, "update", "XRun_Bonus_Cut_Melee_Init", function(self, t, dt)
	if managers.blackmarket._fake_equipped_melee_weapon then
		if self._remove_fake_equipped_melee_weapon then
			...
			if self:X_is_Bonus_Cut_Melee() and col_ray and col_ray.unit and alive(col_ray.unit) and managers.enemy:is_enemy(col_ray.unit) then
				...
				if character_unit:character_damage() and character_unit:character_damage().damage_melee then
					...
					action_data.damage = character_unit:character_damage()._health * 100
				...
				if inventory then
					for ii, weapon in pairs(inventory:available_selections()) do
						weapon.unit:base():add_ammo_from_bag(1)
						weapon.unit:base():on_reload()
						managers.hud:set_ammo_amount(ii, weapon.unit:base():ammo_info())
					end
				...
				HudChallengeNotification.queue(
					'['..managers.localization:text('bm_askn_dewmslaya_b_cr_popup_name')..']',
					tostring(self._Bonus_Cut_Melee_Times).." --> "..tostring(self._Bonus_Cut_Melee_Times-1),
					'pd2_chainsaw'
				)
...
local XBonus_Cut_Melee_Lock_1 = PlayerStandard._check_action_melee

function PlayerStandard:_check_action_melee(...)
	if self:X_is_Bonus_Cut_Melee() then
		return
	end
	return XBonus_Cut_Melee_Lock_1(self, ...)
end

local XBonus_Cut_Melee_Lock_2 = PlayerStandard._is_meleeing

function PlayerStandard:_is_meleeing(...)
	if self:X_is_Bonus_Cut_Melee() then
		return true
	end
	return XBonus_Cut_Melee_Lock_2(self, ...)
end