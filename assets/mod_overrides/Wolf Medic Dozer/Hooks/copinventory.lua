...
Hooks:PostHook(CopInventory, "CUS_set_mask_visibility", "WolfMedicDozer_Post_CUS_set_mask_visibility", function(self)
	local mesh_obj = self._unit:get_object(Idstring("g_medicbag"))
	if mesh_obj then
		mesh_obj:set_visibility(true)
	end
	local unit_damage = self._unit:damage()
	if unit_damage and unit_damage:has_sequence("var_model_07") then
		unit_damage:run_sequence_simple("var_model_07")
	end
end)